		...
		<script>
			$("#submit").button().click(
				function(event) {
					...
					//shallow validation of email address (UI)
					if (/^([\w.]+)@([\w.]+)$/.test(user) == false) {
					...
					$.get(url,function(html) {
						if (/^ERR/.test(html) == true) {
							...
							// If "OK"
							var theName = /^OK:(.+):(.+)$/.exec(html);						
							var firstName = theName[1];
							var lastName = theName[2];
							$("#msg").html("<b>Success!</b> Hello, " + firstName + ".");
